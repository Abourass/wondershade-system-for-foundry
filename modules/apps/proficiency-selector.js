...
export default class ProficiencySelector extends TraitSelector {
  ...
  static async getChoices(type, chosen = []) {
    let data = Object.entries(CONFIG.wondershade[`${type}Proficiencies`]).reduce((obj, [key, label]) => {
      obj[key] = { label: label, chosen: chosen.includes(key) };
      return obj;
    }, {});

    const ids = CONFIG.wondershade[`${type}Ids`];
    const map = CONFIG.wondershade[`${type}ProficienciesMap`];
    if (ids !== undefined) {
    ...
    if (type === 'tool') {
      data.vehicle.children = Object.entries(CONFIG.wondershade.vehicleTypes).reduce((obj, [key, label]) => {
        obj[key] = { label: label, chosen: chosen.includes(key) };
        return obj;
      }, {});
      data = game.wondershade.utils.sortObjectEntries(data, 'label');
    }

    for (const category of Object.values(data)) {
      if (!category.children) continue;
      category.children = game.wondershade.utils.sortObjectEntries(category.children, 'label');
    }
   ...
   * in `wondershade.sourcePacks.ITEMS`.
   *
  ...
  static getBaseItem(identifier, { indexOnly = false, fullItem = false } = {}) {
    let pack = CONFIG.wondershade.sourcePacks.ITEMS;
