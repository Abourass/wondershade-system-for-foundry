...
export class AdvancementSelection extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ['wondershade', 'sheet', 'advancement'],
      template: 'systems/wondershade/templates/advancement/advancement-selection.hbs',
      title: 'WonderSystem.AdvancementSelectionTitle',
      width: 500,
      height: 'auto',
    });
  }
  ...
  getData() {
    ...
    for (const advancement of Object.values(game.wondershade.advancement.types)) {
    ...
    data.types = game.wondershade.utils.sortObjectEntries(data.types, 'label');
   ...
   * been resolved.
   * @param {WonderItem} item                         Item to which the advancement should be added.
   * @param {object} [config={}]
   ...
   * @returns {Promise<AdvancementConfig|null>}   Result of `WonderItem#createAdvancement`.
   */
  static async createDialog(item, { rejectClose = false, options = {} } = {}) {
    return new Promise((resolve, reject) => {
      const dialog = new this(item, {
        title: `${game.i18n.localize('WonderSystem.AdvancementSelectionTitle')}: ${item.name}`,
        buttons: {
          submit: {
            callback: (html) => {
