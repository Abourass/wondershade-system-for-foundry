...
export class ScaleValueAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 60,
      icon: 'systems/wondershade/icons/svg/scale-value.svg',
      title: game.i18n.localize('WonderSystem.AdvancementScaleValueTitle'),
      hint: game.i18n.localize('WonderSystem.AdvancementScaleValueHint'),
      multiLevel: true,
   ...
   */
  static TYPES = {
    string: 'WonderSystem.AdvancementScaleValueTypeString',
    number: 'WonderSystem.AdvancementScaleValueTypeNumber',
    dice: 'WonderSystem.AdvancementScaleValueTypeDice',
    distance: 'WonderSystem.AdvancementScaleValueTypeDistance',
  };
  ...
  formatValue(level) {
    ...
    const value = this.valueForLevel(level);
    if (value == null) return null;
    return `${value.value} ${CONFIG.WonderSystem.movementUnits[this.data.configuration.distance.units]}`;
  }
}
...
export class ScaleValueConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ['wondershade', 'advancement', 'scale-value', 'two-column'],
      template: 'systems/wondershade/templates/advancement/scale-value-config.html',
      width: 540,
    });
  }
  ...
  getData() {
    ...
    data.typeHint = game.i18n.localize(`WonderSystem.AdvancementScaleValueTypeHint${config.type.capitalize()}`);
    ...
    data.movementUnits = CONFIG.WonderSystem.movementUnits;
    return data;
  }
  ...
  _prepareLevelData() {
    let lastValue = null;
    return Array.fromRange(CONFIG.WonderSystem.maxLevel + 1).slice(1).reduce((obj, level) => {
  ...
  async _updateObject(event, formData) {
    ...
    if (typeChange && (formData['data.configuration.type'] !== this.advancement.data.configuration.type)) {
      ...
      Array.fromRange(CONFIG.WonderSystem.maxLevel + 1).slice(1).forEach((l) => { formData[`data.configuration.scale.${l}`] = null; });
    }
...
export class ScaleValueFlow extends AdvancementFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: 'systems/wondershade/templates/advancement/scale-value-flow.html',
    });
  }
